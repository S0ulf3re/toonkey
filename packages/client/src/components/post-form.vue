...
>
	<header>
		<button v-if="!fixed" class="cancel _button" @click="cancel"><i class="ph ph-x-bold"></i></button>
		...
		<div>
			...
			<button ref="visibilityButton" v-tooltip="i18n.ts.visibility" class="_button visibility" :disabled="channel != null" @click="setVisibility">
				...
				<span v-if="visibility === 'home'"><i class="ph ph-house-bold"></i></span>
			...
			<button class="submit _buttonGradate" :disabled="!canPost" data-cy-open-post-form-submit @click="post">{{ submitText }}<i :class="reply ? 'ph ph-arrow-bend-up-left-bold' : renote ? 'fas fa-quote-right' : 'fas fa-paper-plane'"></i></button>
		</div>
	</header>
	<div class="form" :class="{ fixed }">
		<XNoteSimple v-if="reply" class="preview" :note="reply"/>
		<XNoteSimple v-if="renote" class="preview" :note="renote"/>
		<div v-if="quoteId" class="with-quote"><i class="fas fa-quote-left"></i> {{ i18n.ts.quoteAttached }}<button @click="quoteId = null"><i class="ph ph-x-bold"></i></button></div>
		<div v-if="visibility === 'specified'" class="to-specified">
			<span style="margin-right: 8px;">{{ i18n.ts.recipient }}</span>
			<div class="visibleUsers">
				<span v-for="u in visibleUsers" :key="u.id">
					<MkAcct :user="u"/>
					<button class="_button" @click="removeVisibleUser(u)"><i class="ph ph-x-bold"></i></button>
				</span>
				<button class="_buttonPrimary" @click="addVisibleUser"><i class="ph ph-plus-bold fa-fw"></i></button>
			</div>
		</div>
		...
		<footer>
			...
			<button v-tooltip="i18n.ts.mention" class="_button" @click="insertMention"><i class="ph ph-at-bold"></i></button>
			<button v-tooltip="i18n.ts.hashtags" class="_button" :class="{ active: withHashtags }" @click="withHashtags = !withHashtags"><i class="ph ph-hash-boldtag"></i></button>
